// Simple program to compare matrixes


#include <stdio.h>
#include <stdlib.h>

#define N 1024
int correctC[N][N], C[N][N];
int readmat(char *fname, int *mat, int n), 
    writemat(char *fname, int *mat, int n);


int main(){ 

	int i,j;

	printf("Comparing\n...\n");

	/* Read matrices from files: "A_file", "B_file" 
	 */
	if (readmat("correctC", (int *) correctC, N) < 0) 
		exit( 1 + printf("file problem\n") );
	if (readmat("Cmat1024", (int *) C, N) < 0) 
		exit( 1 + printf("file problem\n") );


	for (i = 0; i < N; i++)
	{
		for (j = 0; j < N; j++)
		{
			if (correctC[i][j] != C[i][j])
			{
				printf("Output matrix C is incorrect\n");
				exit(0);
			};
		};
	};

	printf("Output matrix C is correct\n");

	

}




#define _mat(i,j) (mat[(i)*n + (j)])


int readmat(char *fname, int *mat, int n)
{
	FILE *fp;
	int  i, j;
	
	if ((fp = fopen(fname, "r")) == NULL)
		return (-1);
	for (i = 0; i < n; i++)
		for (j = 0; j < n; j++)
			if (fscanf(fp, "%d", &_mat(i,j)) == EOF)
			{
				fclose(fp);
				return (-1); 
			};
	fclose(fp);
	return (0);
}


int writemat(char *fname, int *mat, int n)
{
	FILE *fp;
	int  i, j;
	
	if ((fp = fopen(fname, "w")) == NULL)
		return (-1);
	for (i = 0; i < n; i++, fprintf(fp, "\n"))
		for (j = 0; j < n; j++)
			fprintf(fp, " %d", _mat(i, j));
	fclose(fp);
	return (0);
}
